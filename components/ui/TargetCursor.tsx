...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const spinTl = useRef<any | null>(null);
  ...
  const moveCursor = useCallback(
    (x: number, y: number) => {
      if (!cursorRef.current || !gsapRef.current) return;
    ...
    import("gsap").then((mod) => {
      ...
      const enterHandler = (e: MouseEvent) => {
        ...
        corners.forEach((corner, i) => {
          gsap.to(corner, {
            x:
              targetCornerPositionsRef.current![i].x -
              cursorX,
            y:
              targetCornerPositionsRef.current![i].y -
              cursorY,
            duration: 0.2,
            ease: "power2.out",
          });
  ...
  useEffect(() => {
    if (isMobile || !cursorRef.current || !spinTl.current || !gsapRef.current) return;
    if (spinTl.current.isActive()) {
      spinTl.current.kill();
      spinTl.current = gsapRef.current
        .timeline({ repeat: -1 })
        .to(cursorRef.current, {
          rotation: "+=360",
          duration: spinDuration,
